...
class DeepseekSparseSWAMetadataBuilder(AttentionMetadataBuilder):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Sized like the decode buffers (num_prefill_tokens <= max tokens).
    ...
    def build(
        ...
        # Prefill SWA indices for the SM120 run_sparse_mla prefill path. The local
        # triton kernel above is DECODE-ONLY: it uses program_id(0) as an absolute
        # token index in `pos = prefix_len + token_idx - query_start`, so it cannot
        # be reused with a prefill-local grid. FlashInfer's compute_swa_indices_and_lens
        # takes a `token_offset` that reads is_valid_token/token_to_req_indices at
        # absolute positions [num_decode_tokens:] while writing output at [0:N] — the
        # exact path lucifer1004 validated. (Helper present from the swapped flashinfer.)
